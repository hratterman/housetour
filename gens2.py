...
class Gens2:
    ...
    def gen_kitchen2(self, e):
        ...
        for ly in (20.2, 21.8):   # two small LED lamps, not a 3.6 sq ft light panel (which blew out under the camera at t 15)
            objs.append(box_ft(self.uid("k_hood_lens"), wx - 1.35, ly - 0.2, wx - 0.95, ly + 0.2, z + 5.98, z + 6.0, self.mat("hood_lamp"), self.col))
        ...
        for cy in (23.25, 25.0, 26.75):
            objs += self.gen_dining_chair({"pos": [12.35, cy, z], "rot_z": 90, "m": "wool_mustard"})    # west side, faces +X into the table
