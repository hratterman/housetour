...
def main_floor():
    ...
    proc("stroller", "gear_closet", pos=[4.5, 2.3, 0], rot_z=0)
    proc("bike", "gear_closet", pos=[1.9, 3.4, 0], rot_z=90, wheel_r=0.7, m="oxblood")   # one kid bike fits the 4.75 ft depth; the other hangs in the garage
    ...
    model("rubber_boots", (9.05, 4.9, 0), "vestibule", height_ft=1.1, rot_z=20)
    ...
    # ------------------------------------------------------------------ 3.8 stair hall / well
    note("stair hall")
    proc("globe_pendant", "stair_hall", pos=[29.75, 5.5, 13.5], radius=0.83, drop=4.7, watts=70)   # in the two-storey well over the west flight (9 ft above its treads), canopy on the second-floor ceiling
    ...
    add(asset="proc:cabinet", room="laundry", b=[35.5, 1.0, 39.0, 3.0, 0.3, 2.9], doors=3, face="+y", m="walnut_h")   # the sink cabinet takes X 39-41
    ...
    model("rubber_boots", (2.05, 18.85, 0), "mudroom", height_ft=1.1, rot_z=92)
    # ------------------------------------------------------------------ 3.12 litter closet
    note("litter closet")
    ...
    # low slatted walnut media cabinet (Poly Haven modern_wooden_cabinet, 5 ft: the column and the pocket door bound it), top at 1.39
    model("modern_wooden_cabinet", (19.0, 33.3, 0), "living", length_ft=5.0, rot_z=90)
    proc("records", "living", pos=[19.0, 31.1, 1.39], rot_z=90, count=8)
    proc("speaker", "living", pos=[19.15, 35.2, 1.39], rot_z=90, face="-y")
    proc("speaker", "living", pos=[19.15, 31.5, 1.39], rot_z=90, face="-y")
    proc("table_lamp", "living", pos=[19.05, 35.4, 1.39], height=1.8, base_r=0.22, shade_r=0.45, base_m="brass", watts=25)
    proc("tv_wall", "living", wall=W("y", 20.0, "-x"), u=33.0, zc=4.6, w=4.8, h=2.6, on=True, seed=88, watts=8)
    frames("living", W("y", 20.0, "-x"), [30.6, 35.9], 6, zc=7.3, seed=61)   # above the TV (Z 3.3-5.9)
    ...
    proc("plant", "living", pos=[18.3, 44.2, 0], kind="monstera", height=5.5, seed=4, avoid=[0, 90])   # spec: a large monstera in the NE corner
    proc("plant", "living", pos=[6.7, 44.4, 0], kind="ficus", height=5.0, seed=5, avoid=[90])      # spec: a rubber plant by the shelves
    ...
    model("mid_century_lounge_chair", (25.0, 42.8, 0), "away", length_ft=2.9, rot_z=0)   # faces the ottoman (models face -Y at rot 0)
    model("ottoman_01", (25.0, 40.3, 0), "away", length_ft=2.1, rot_z=180, recolor=[0.3, 0.18, 0.1])
    proc("mushroom_lamp", "away", pos=[21.3, 44.0, 0], height=5.0, m="brass", watts=38)   # west of the cabinet, beside the chair
    proc("bookwall", "away", b=[26.75, 30.6, 27.75, 44.9, 0.0, 9.0], face="-x", seed=7, density=0.92, shelf_ft=1.2)
    model("side_table_01", (22.8, 42.8, 0), "away", height_ft=1.9)
    ...
    add(asset="proc:cabinet", room="away", b=[22.5, 44.2, 26.6, 45.0, 0, 2.5], doors=3, face="-y", m="walnut_h")
    ...
    proc("toilet", "wc", pos=[39.5, 17.3, 0], facing="-y", bidet=True)   # on the north wall, facing down the 4.5 ft length; 18 in each side
    proc("wall_shelf", "wc", wall={"axis": "y", "at": 41.0, "face": "-x"}, u=16.2, z=3.6, length=1.2, depth=0.45, items=["roll", "candle"])   # small brass shelf on the east wall, a spare roll and a candle
    ...
    model("potted_plant_04", (33.0, 20.6, 0), "primary_bath", height_ft=2.2)
    ...
    proc("platform_bed", "primary_bedroom", pos=[32.15, 38.0, 0], width=6.5, length=7.0, rot_z=90, platform_h=1.0, mattress_top=2.0,
    ...
    proc("nightstand2", "primary_bedroom", pos=[29.05, 32.4, 0], rot_z=90, on=False, items=["books", "glass"])
    ...
    proc("plant", "primary_bedroom", pos=[31.2, 43.95, 0], kind="ficus", height=5.0, seed=8, avoid=[90])   # spec: a tall plant in the corner   # SE corner, out of the bedroom shot path
