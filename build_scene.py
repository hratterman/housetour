...
def bevel_pass(plan):
    ...
    for ob in bpy.data.objects:
        ...
        if ob.data.users > 1 and not nm.startswith(("wall_", "floor_", "ceil_")):
            continue
        if nm.startswith("car_cover_"):
            w, seg = 3.0 / 12.0 * FT, 4
            cloth.append(ob)                     # the fitted cover wrinkles like fabric
        elif nm.startswith(("car_body", "car_cabin", "car_roof")):
            w, seg = 3.0 / 12.0 * FT, 4          # sheet metal: a soft radius turns the extruded profile into a body
...
def audit_staging(plan, out_path):
    ...
    exterior = ("porch", "soffit", "house_numbers", "grill", "heater", "planter", "rain_chain", "spa", "outdoor", "ceiling_fan",
                ...
                "fern_02", "exterior_aircon_unit", "garden_gnome", "propane_tank", "stone_fire_pit", "watering_can_metal_01", "wooden_picnic_table")
...
def key_shot(scene, cam, tgt, shot, fps, base_exposure=None, override=None):
    ...
    if scene.animation_data and scene.animation_data.action:
        for fc in [fc for fc in scene.animation_data.action.fcurves if fc.data_path == "view_settings.exposure"]:
