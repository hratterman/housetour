...
and stitches a walkthrough video. Everything is regenerated from JSON; nothing is hand-modeled in a .blend
file. `plan.json` itself is generated by `tools/make_plan.py` from `housemasterspec.md` (do not hand-edit it).
...
The build itself takes about 30 s; the log prints the per-still render time. Multiply by 2,592 frames
(108 s of shots at 24 fps). If a frame is slower than about 90 s, `SAMPLES=160` is a fine compromise; denoising
